...
resource "aws_security_group" "rds_sg" {
  vpc_id      = "vpc-0216325cc39bdd220" # default VPC
  name        = "allow_mysql"
  ...
  ingress {
    ...
    to_port         = 3306
    protocol        = "tcp"
    cidr_blocks     = ["191.91.55.29/32"]           # local IP address
    security_groups = [aws_security_group.tf_ec2_sg2.id] # Allow traffic from EC2 security group
  }
